    ...
    <v-data-table-server v-model:items-per-page="itemsPerPage" :headers="headers" :items="serverItems"
        ...
        <template v-slot:top>
            <v-toolbar flat style="background-color: #006398; color: white;">
                <v-toolbar-title>Armado kardex<v-icon class="mx-2">mdi-package-variant-closed</v-icon></v-toolbar-title>
            </v-toolbar>
        </template>
        <template v-slot:[`item.FECHA_HRS_OPERACION`]="{ item }">
            <span>{{ formatearFecha(item.FECHA_HRS_OPERACION) }}</span>
        </template>
...
export default {
    data: () => ({
        ...
        headers: [
            ...
            { title: 'Saldo', key: 'SAL_ACU_XARMA', align: 'start', sortable: false },
            ...
            { title: 'Acumulado por despachar', key: 'SAL_ACU_XDESPACHAR', align: 'start', sortable: false },
            { title: 'Tipo operación', key: 'TIPO_OPERACION', align: 'start', sortable: false },
            { title: 'Fecha', key: 'FECHA_HRS_OPERACION', align: 'start', sortable: false },
        ],
        serverItems: [],
        loading: true,
        totalItems: 0,
    ...
    methods: {
        async loadItems({ page, itemsPerPage } = {}) {
            ...
            try {
                ...
                await axios.get(`/armadokardex/paginado`, {
                    params: {
                        take: itemsPerPage,
                        page: page,
                        rut: this.rutCliente,
                    ...
                    this.serverItems = response.data.armadoKardex;
