<template>
    <v-container fluid class="d-flex">
        ...
        <v-col cols="12" lg="2" md="3" sm="3">
            <v-select prepend-inner-icon="mdi-magnify" v-model="filtroAdhesivo" label="Adhesivo" chips
                clearable :items="adhesivos" item-title="codigo" item-value="id">
                <template v-slot:item="{ props, item }">
                    <v-list-item v-bind="props"
                        :subtitle="item.raw.descripcion"></v-list-item>
                </template>
            </v-select>
        </v-col>
        <v-col cols="12" lg="2" md="3" sm="3">
            <v-select prepend-inner-icon="mdi-magnify" v-model="filtroOperacion" label="Operación" chips
                clearable :items="operaciones">
            </v-select>
        </v-col>
        <v-col cols="12" lg="2" md="3" sm="3">
            <v-text-field prepend-inner-icon="mdi-magnify" v-model="filtroGuia" label="Guía de despacho" hide-details
                clearable></v-text-field>
        </v-col>
        <v-col cols="12" lg="2" md="3" sm="3">
            <v-locale-provider locale="es">
                <v-date-input v-model="rangoFecha" label="Fecha" prepend-icon=""
    ...
    <v-data-table-server v-model:items-per-page="itemsPerPage" :headers="headers" :items="serverItems"
        ...
        <template v-slot:top>
            <v-toolbar flat style="background-color: #006398; color: white;">
                <v-toolbar-title>Adhesivo por prestador<v-icon class="mx-2">mdi-water</v-icon></v-toolbar-title>
            </v-toolbar>
        </template>
...
export default {
    data: () => ({
        ...
        headers: [
            ...
            { title: 'Prestador ID', key: 'prestadorId', align: 'start', sortable: false },
            ...
            { title: 'Kilos entregados', key: 'kilosEntregados', align: 'start', sortable: false },
            ...
            { title: 'Kilos no asignados', key: 'kilosNoAsignados', align: 'start', sortable: false },
    ...
    methods: {
        async loadItems({ page, itemsPerPage } = {}) {
            ...
            try {
                ...
                await axios.get(`/adhesivokardex/prestador`, {
                    params: {
                        take: itemsPerPage,
                        page: page,
                    ...
                    this.serverItems = response.data.adhesivosPrestadorKardex;
