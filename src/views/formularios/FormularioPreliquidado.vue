                    ...
                    <v-card class="cartas">
                        ...
                        <v-row justify="center" style="margin-top: 20px;">
                            <v-col cols="12" lg="2" md="2" sm="2">
                                <v-text-field v-model="nroTroquel" label="N° troquel" variant="solo-filled"></v-text-field>
                            </v-col>
                        </v-row>
                    </v-card>
                    <v-card class="cartas">
    ...
    methods: {
        ...
        async obtenerArmado() {
            try {
                ...
                await axios.get(`/armados/obtener/${this.$route.params.id}`, {
                    ...
                    if (this.formularioAntiguo.estado !== 'aprobado' && this.formularioAntiguo.estado !== 'pre-liquidación') {
                        this.$notify({
        ...
        async guardar() {
            ...
            try {
                if (this.validarFormulario()) {
                    ...
                    await this.recalcularLiquidación();
                    this.$router.go(-1);
                }
            } catch (error) {
        ...
        async recalcularLiquidación() {
            try {
                ...
                await axios.post(`/liquidaciones/recalcularPrecio/${this.formularioAntiguo.liquidaciones[0].id}`, {}, {
                    headers: {
                        Authorization: `Bearer ${token}`
                    }
                }).then(() => {
                    this.$notify({
                        type: 'exito',
                        text: 'Liquidación recalculada'
                    });
                }).catch((error) => {
                    console.error('Error al recalcular la liquidación', error);
                    this.$notify({
                        type: 'error',
                        text: 'Error al recalcular la liquidación'
                    });
                });
            } catch (error) {
                console.error('Error al recalcular la liquidación', error);
